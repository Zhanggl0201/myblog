	...
	if (isExternalMode) {
		...
		if (!hasWebpCover) {
			...
			if (!fs.existsSync(coverPath)) {
				coverPath = path.join(folderPath, "cover.jpeg");
				if (!fs.existsSync(coverPath)) {
					coverPath = path.join(folderPath, "cover.png");
					if (!fs.existsSync(coverPath)) {
		...
		cover = hasWebpCover
			...
			: `/images/albums/${folderName}/cover${path.extname(coverPath)}`;
...
function scanPhotos(folderPath: string, albumId: string): Photo[] {
	...
	imageFiles.forEach((file, index) => {
		...
		// 如果该文件有对应的 webp 版本，跳过原文件（webp 文件自身会被遍历到）
		if (fileWebpMap.has(file)) {
			return;
		}

		const src = `/images/albums/${albumId}/${file}`;
